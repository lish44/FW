...
namespace FW
{
    public static class FUtils
    {
        ...
        public static List<FResourceRef> AddTargetToList(List<FResourceRef> targetList, List<FResourceRef> list)
        {
            if (null == targetList || null == list)
            {
                Debug.LogError("param can not be null.");
        ...
        public static Dictionary<string, Dictionary<string, List<FResourceRef>>> AddTargetToDic(
            Dictionary<string, Dictionary<string, List<FResourceRef>>> targetDic,
          Dictionary<string, Dictionary<string, List<FResourceRef>>> dic)
        {
            if (null == targetDic || null == dic)
            {
                Debug.LogError("param can not be null.");
                return null;
            }

            foreach (var valueDic in dic.Values )
            {
                foreach (var valueTargetDic in targetDic.Values)
                {
                    int _count = valueDic.Keys.Count;
                    string[] _array = new string[_count];
                    valueDic.Keys.CopyTo(_array, 0);
                    for (int j = 0; j < _array.Length; ++j)
                    {
                        string _key = _array[j];
                        List<FResourceRef> _value = null;
                        if (!valueTargetDic.TryGetValue(_key, out _value))
                        {
                            _value = new List<FResourceRef>();
                            valueTargetDic.Add(_key, _value);
                        }

                        _value = AddTargetToList(_value, valueDic[_key]);
                    }
                }
            }
